...
def func_parse_parameter(sys_argv, str_fname_output):
    if len(sys_argv) == 2 :
        if sys_argv[1] == '-h' \
            or sys_argv[1] == '--help':
            print(sys_argv[0], ' code=[銘柄コード] date=[yyyymmdd]')
            print()
            print('sample')
            print(sys_argv[0], 'code=94320 date=20220808')
            print(sys_argv[0], 'code=94320')
            print(sys_argv[0], 'code=94320 date=')
            print(sys_argv[0], 'date=20220808')
            print(sys_argv[0], 'code= date=20220808')
            exit()
        else :
            pass
        
    elif len(sys_argv) == 1 or len(sys_argv) > 4 :
        print(sys_argv[0], ' code=[銘柄コード] date=[yyyymmdd]')
        print()
        print('sample')
        print(sys_argv[0], 'code=94320 date=20220808')
...
def func_query_api(str_idToken, str_url):
    ...
    ##resp = requests.get("https://api.jpx-jquants.com/v1/fins/statements?code=86970", headers=headers)
...
#
# 使い方:
#   プログラム名 code=[銘柄コード5桁] date=[日付(yyyymmdd)]
#   銘柄コードまたは日付の一方の省略は可能
#   ./jq_api_get_fins_statements.py code=94320 date=20220808
#   ./jq_api_get_fins_statements.py code=94320
#   ./jq_api_get_fins_statements.py code=94320 date=
#   ./jq_api_get_fins_statements.py date=20220808
#   ./jq_api_get_fins_statements.py code= date=20220808
#
# =============================================
...
# 出力ファイル名
str_fname_output = 'jq_fins_statements.csv'

# パラメーターチェック
func_parse_parameter(sys.argv, str_fname_output)

# 財務情報を取得する銘柄コード
str_parameter = sys.argv[1] + '&' + sys.argv[2]
    ...
    with open(str_fname_output, 'a', encoding = 'utf_8') as fout:
        if len(list_statements) > 0 :
            ...
            for i in range(len(list_statements)):
                ...
                fout.write(str_text)     
            print('データ数: ', i + 1 )  # 0からカウントしているので1加算
        else :
            print('対象データなし')
